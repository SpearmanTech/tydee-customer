...
export const onNewChatMessage = onDocumentCreated(
  ...
  async (event) => {
    ...
    const recipientId =
      senderId === chatData.ownerId ? chatData.renterId : chatData.ownerId;
    ...
    if (!pushToken) {
      console.log(
        `Execution skipped: No push token on file for user ${recipientId}`,
      );
      return;
    }
    ...
    const payload = {
      token: pushToken,
      notification: {
        title: chatData.equipmentName
          ? `New message about ${chatData.equipmentName}`
          : "Foona Message",
    ...
    try {
      ...
      console.log(
        `Push notification successfully dispatched to ${recipientId}`,
      );
    } catch (error) {
