...
// 1. Move the interface and component BEFORE they are used in ListStepFour
...
function HandoverOption({ active, title, sub, icon, onPress }: HandoverOptionProps) {
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.7}>
      <View style={[styles.optionCard, active && styles.activeOption]}>
        ...
        <View style={{ flex: 1, marginLeft: 15 }}>
          <Text style={[styles.optionTitle, active && styles.activeOptionTitle]}>{title}</Text>
          <Text style={styles.optionSub}>{sub}</Text>
        </View>
        {active && <Ionicons name="checkmark-circle" size={24} color="#4f46e5" />}
      </View>
    </TouchableOpacity>
  );
}
 ...
 const handlePublish = async () => {
  ...
  try {
    ...
    const response = await publishListingFn({
      ...
      images: JSON.parse(params.images as string), // Ensure this is an array
      dailyRate: params.dailyRate,
      deposit: params.deposit,
      lat: -29.8579, // You can make these dynamic later
      lng: 31.0292,
      area: "Umhlanga, Durban",
      handoverType: deliveryOption, // 'pickup' or 'both'
    });

    if (response.data.success) {
      Alert.alert("Success!", "Your gear is live on Tydee.");
    ...
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.sub}>Finalize where your gear lives and how it reaches customers.</Text>
        </View>
        ...
        <View style={styles.mapCard}>
          <View style={styles.mapWrapper}>
            <MapView
              ...
              scrollEnabled={false}
            >
              <Marker coordinate={{ latitude: -29.8579, longitude: 31.0292 }}>
          ...
          <TouchableOpacity style={styles.locationInfo}>
             ...
             <Text style={styles.addressText}>Umhlanga Rocks Dr, Durban</Text>
        ...
        <View style={styles.section}>
          ...
          <HandoverOption 
            ...
            icon="home-outline"
            onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setDeliveryOption('pickup'); }}
          />
          ...
          <HandoverOption 
            ...
            icon="car-outline"
            onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setDeliveryOption('both'); }}
          />
        </View>
        ...
        <View style={styles.privacyCard}>
           ...
           <View style={styles.privacyTextContent}>
              <Text style={styles.privacyTitle}>Tydee Privacy Shield</Text>
              <Text style={styles.privacySub}>Your exact address is only shared with verified renters after they pay the deposit.</Text>
           </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  mapCard: { margin: 24, borderRadius: 32, backgroundColor: 'white', overflow: 'hidden', borderWidth: 1, borderColor: '#f1f5f9', shadowColor: '#000', shadowOpacity: 0.03, shadowRadius: 15 },
  mapWrapper: { height: 180, width: '100%' },
  map: { ...StyleSheet.absoluteFillObject },
  customMarker: { width: 30, height: 30, borderRadius: 15, backgroundColor: 'rgba(99, 102, 241, 0.2)', justifyContent: 'center', alignItems: 'center' },
  markerPulse: { width: 10, height: 10, borderRadius: 5, backgroundColor: '#4f46e5', borderWidth: 2, borderColor: 'white' },
  locationInfo: { padding: 20, flexDirection: 'row', alignItems: 'center' },
  addressText: { flex: 1, marginLeft: 10, fontSize: 14, fontWeight: '600', color: '#0f172a' },
  editText: { color: '#4f46e5', fontWeight: '800', fontSize: 13 },

  section: { paddingHorizontal: 24, marginTop: 10 },
  sectionLabel: { fontSize: 10, fontWeight: '900', color: '#6366f1', letterSpacing: 1, marginBottom: 15 },
  optionCard: { flexDirection: 'row', alignItems: 'center', padding: 18, backgroundColor: 'white', borderRadius: 24, marginBottom: 12, borderWidth: 1, borderColor: '#f1f5f9' },
  ...
  privacyCard: { flexDirection: 'row', margin: 24, padding: 20, backgroundColor: '#ecfdf5', borderRadius: 24, alignItems: 'flex-start' },
  privacyTextContent: { flex: 1, marginLeft: 15 },
  privacyTitle: { fontSize: 14, fontWeight: '800', color: '#065f46' },
  privacySub: { fontSize: 12, color: '#065f46', opacity: 0.8, marginTop: 4, lineHeight: 18 },

  footer: { position: 'absolute', bottom: 40, width: '100%', paddingHorizontal: 24 },
  publishBtn: { borderRadius: 24, overflow: 'hidden' },
  gradient: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 20, gap: 12 },
  publishText: { color: 'white', fontWeight: '800', fontSize: 17 }
});

function setIsPublishing(arg0: boolean) {
