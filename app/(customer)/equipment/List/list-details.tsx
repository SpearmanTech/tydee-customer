...
export default function ListStepTwo() {
  ...
  const handlePickImage = async (slotId: string) => {
    ...
    if (status !== "granted") {
      Alert.alert(
        "Permission needed",
        "Foona needs access to your photos to list gear.",
      );
      return;
    }
  ...
  const handleGammaVerifyAndSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Gamma Sync Failed",
        "We couldn't verify your images. Check your connection.",
      );
    } finally {
      setScanningSlot(null);
  ...
  const isComplete =
    ...
    Object.keys(selectedImages).length === 4;
    ...
    >
          ...
          <View style={styles.photoGrid}>
            {PHOTO_SLOTS.map((slot) => (
              <TouchableOpacity
                ...
                style={[
                  styles.slotCard,
                  scanningSlot === slot.id && styles.scanningCard,
                ]}
              ...
              >
                {selectedImages[slot.id] ? (
                  <Image
                    source={{ uri: selectedImages[slot.id] }}
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          style={[
            styles.nextBtn,
            (!isComplete || isUploading) && styles.disabledBtn,
          ]}
        ...
        >
          <LinearGradient
            colors={
              isComplete ? ["#6366f1", "#4f46e5"] : ["#e2e8f0", "#cbd5e1"]
            }
