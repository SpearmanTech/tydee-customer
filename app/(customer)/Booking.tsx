  ...
  useEffect(() => {
    const initializeBooking = async () => {
      try {
        ...
        const serviceSnap = await getDoc(doc(db, "services", categoryId as string));
        ...
        if (serviceSnap.exists()) {
          const data = serviceSnap.data()[subServiceId as string] || serviceSnap.data().subServices?.[subServiceId as string];
          ...
          setFormData(prev => {
            ...
            config.forEach(f => {
              if (!(f.key in initial)) initial[f.key] = f.type === 'counter' ? 0 : false;
            });
            return initial;
          });
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        <View style={styles.header}>
            ...
            <Text style={styles.headerTitle}>{serviceDetails?.name || "Booking"}</Text>
            <View style={{width: 28}} />
        </View>
          ...
          {step === 1 ? (
            <Animated.View entering={FadeInDown}>
              ...
              <View style={styles.card}>
                {(SUB_SERVICE_CONFIGS[subServiceId as string] || []).map((field) => (
                  <View key={field.key} style={styles.fieldWrapper}>
                    {field.type === 'counter' ? (
                      <View style={styles.counterRow}>
                        <Text style={styles.fieldLabel}>{field.label}</Text>
                        <View style={styles.counterActions}>
                          <TouchableOpacity onPress={() => setFormData({...formData, [field.key]: Math.max(0, formData[field.key]-1)})} style={styles.cBtn}><Ionicons name="remove" size={18}/></TouchableOpacity>
                          <Text style={styles.cValue}>{formData[field.key]}</Text>
                          <TouchableOpacity onPress={() => setFormData({...formData, [field.key]: formData[field.key]+1})} style={styles.cBtn}><Ionicons name="add" size={18}/></TouchableOpacity>
                        </View>
                      </View>
                    ) : (
                      <TouchableOpacity 
                        style={[styles.toggleBtn, formData[field.key] && styles.toggleActive]}
                        onPress={() => setFormData({...formData, [field.key]: !formData[field.key]})}
                      >
                        <Text style={[styles.toggleText, formData[field.key] && styles.toggleTextActive]}>{field.label}</Text>
                        <Ionicons name={formData[field.key] ? "checkbox" : "square-outline"} size={22} color={formData[field.key] ? "#fff" : "#6b7280"} />
                      </TouchableOpacity>
                    )}
                  </View>
                ))}
              ...
              <TouchableOpacity style={styles.primaryBtn} onPress={() => setStep(2)}>
                <Text style={styles.primaryBtnText}>Next: Location & Urgency</Text>
              </TouchableOpacity>
            </Animated.View>
          ) : (
            <Animated.View entering={FadeInDown}>
              ...
              <View style={styles.urgencyContainer}>
                {[
                  { id: 'urgent', label: 'Urgent', desc: 'Within 2 hrs', mult: '1.5x' },
                  { id: 'standard', label: 'Standard', desc: 'Within 6 hrs', mult: '1.2x' },
                  { id: 'scheduled', label: 'Schedule', desc: 'Pick a date', mult: '1.0x' },
                ].map((item) => (
                  <TouchableOpacity 
                    key={item.id}
                    style={[styles.urgencyCard, formData.urgency === item.id && styles.urgencyActive]}
                    onPress={() => setFormData({...formData, urgency: item.id})}
                  >
                    <Text style={[styles.urgencyTitle, formData.urgency === item.id && styles.whiteText]}>{item.label}</Text>
                    <Text style={[styles.urgencyDesc, formData.urgency === item.id && styles.whiteText]}>{item.desc}</Text>
              ...
              <View style={styles.priceCard}>
                <Text style={styles.priceValue}>{formatCurrency(calculateTotal())}</Text>
                <Text style={styles.priceLabel}>Estimated Total</Text>
              </View>

              <TouchableOpacity style={styles.submitBtn} onPress={handleSubmit} disabled={submitting}>
                {submitting ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitBtnText}>Post Job Request</Text>}
              </TouchableOpacity>
            </Animated.View>
          )}
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 12, fontWeight: "800", color: "#6b7280", textTransform: "uppercase", letterSpacing: 1, marginBottom: 15 },
  card: { backgroundColor: "#fff", padding: 16, borderRadius: 20, borderWidth: 1, borderColor: "#e5e7eb", marginBottom: 20 },
  ...
  cBtn: { width: 32, height: 32, borderRadius: 16, backgroundColor: "#f3f4f6", justifyContent: "center", alignItems: "center" },
  cValue: { fontSize: 18, fontWeight: "700" },
  toggleBtn: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", backgroundColor: "#fff", padding: 16, borderRadius: 12, borderWidth: 1, borderColor: "#e5e7eb" },
  ...
  whiteText: { color: '#fff' },
  multiplierBadge: { marginTop: 6, backgroundColor: '#06b6d4', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6 },
  multiplierText: { fontSize: 9, fontWeight: '900', color: '#fff' },
  priceCard: { backgroundColor: "#fff", padding: 24, borderRadius: 20, alignItems: "center", marginVertical: 20, borderStyle: "dashed", borderWidth: 2, borderColor: "#cbd5e1" },
  priceValue: { fontSize: 32, fontWeight: "800", color: "#111827" },
  priceLabel: { fontSize: 14, color: "#6b7280" },
  primaryBtn: { backgroundColor: "#111827", padding: 18, borderRadius: 16, alignItems: "center", marginTop: 20 },
  primaryBtnText: { color: "#fff", fontWeight: "700", fontSize: 16 },
  submitBtn: { backgroundColor: "#06b6d4", padding: 18, borderRadius: 16, alignItems: "center" },
